...
final class WP_Full_MCP_Gateway {
    ...
    private function defaults(): array {
        return [
            ...
            'approval_required_tools' => "wp-elementor-convert-columns-to-containers",
...
wp-import-settings",
    ...
    private function maybe_upgrade_options(): void {
        ...
        if (empty($opts['service_user_id'])) {
            $opts['service_user_id'] = $this->default_service_user_id();
        }
        $opts['approval_required_tools'] = '';
        if ($stored_version === (defined('WPFMCP_VERSION') ? WPFMCP_VERSION : '') && !empty($this->opts['secret']) && !empty($this->opts['service_user_id']) && trim((string)($this->opts['approval_required_tools'] ?? '')) === '') return;
        foreach (['approval_required_tools','backup_before_tools'] as $key) {
            $list = $this->parse_list((string)($opts[$key] ?? ''));
            foreach ($key === 'approval_required_tools' ? [] : ['wp-import-settings','wp-activate-plugin','wp-deactivate-plugin','wp-elementor-kit-erase','wc-create-product','wc-update-product','wc-set-product-pricing','wc-set-product-thumbnail','wc-delete-product','wc-update-order-status','wc-add-order-note','wc-enable-native-mcp'] as $tool) {
    ...
    public function admin_init(): void {
        ...
        if (isset($_POST['wpfmcp_save']) && check_admin_referer('wpfmcp_save')) {
            ...
            $service_user_id = absint($post['service_user_id'] ?? ($opts['service_user_id'] ?? 0));
            $service_user = $service_user_id ? get_userdata($service_user_id) : false;
            $opts['service_user_id'] = $service_user ? $service_user_id : $this->default_service_user_id();
        ...
        <div class="wrap">
            ...
            <form method="post">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr><th scope="row">Service user ID</th><td><input type="number" name="service_user_id" value="<?php echo esc_attr((string)($this->opts['service_user_id'] ?? 0)); ?>" min="1" class="small-text"><p class="description">Remote MCP requests that pass the secret run capability checks as this WordPress user. Use an administrator/editor account that matches the tools you expose.</p></td></tr>
                    ...
                    <tr><th scope="row">Allowed plugin slugs</th><td><textarea name="allowed_plugin_slugs" rows="5" cols="60" placeholder="elementor&#10;classic-editor"><?php echo esc_textarea($this->opts['allowed_plugin_slugs']); ?></textarea><p class="description">Optional newline/comma separated allowlist. Empty = any WordPress.org slug for install/update/delete when enabled.</p></td></tr>
                    ...
                    <tr><th scope="row">Allowed theme slugs</th><td><textarea name="allowed_theme_slugs" rows="5" cols="60" placeholder="twentytwentysix&#10;hello-elementor"><?php echo esc_textarea($this->opts['allowed_theme_slugs']); ?></textarea><p class="description">Optional newline/comma separated allowlist. Empty = any WordPress.org theme slug for install/update/switch when enabled.</p></td></tr>
...
"); } } fclose($fh); return ['strategy'=>'db_export','name'=>$name,'bytes'=>filesize($file),'created_at'=>gmdate('c'),'stored'=>'private_uploads','note'=>'Backup path is intentionally omitted from MCP responses.']; }
    ...
    private function default_service_user_id(): int { $admins=get_users(['role'=>'administrator','number'=>1,'orderby'=>'ID','order'=>'ASC','fields'=>'ID']); return !empty($admins[0]) ? (int)$admins[0] : 0; }
    private function authenticate_service_user(): void { if(get_current_user_id()) return; $user_id=absint($this->opts['service_user_id'] ?? 0); if(!$user_id) $user_id=$this->default_service_user_id(); if(!$user_id || !get_userdata($user_id)) throw new RuntimeException('MCP service user is not configured. Set Service user ID in WP Full MCP settings.'); wp_set_current_user($user_id); }
    ...
    private function allowed_slug(string $slug): bool { $slug=sanitize_key($slug); $list=preg_split('/[\s,]+/', (string)$this->opts['allowed_plugin_slugs'], -1, PREG_SPLIT_NO_EMPTY); if(!$list)return true; return in_array($slug, array_map('sanitize_key',$list), true); }
    ...
    private function allowed_theme_slug(string $slug): bool { $slug=sanitize_key($slug); $list=preg_split('/[\s,]+/', (string)$this->opts['allowed_theme_slugs'], -1, PREG_SPLIT_NO_EMPTY); if(!$list)return true; return in_array($slug, array_map('sanitize_key',$list), true); }
...
8. Publish as draft, then review';
    ...
    private function elementor_kit_file_from_args(array $args): array { $tmp=''; if(!empty($args['kit_url'])){ $url=esc_url_raw((string)$args['kit_url']); if(!preg_match('#^https?://#i',$url)) throw new RuntimeException('kit_url must be http(s).'); require_once ABSPATH.'wp-admin/includes/file.php'; $tmp=download_url($url, 120); if(is_wp_error($tmp)) throw new RuntimeException($tmp->get_error_message()); return [$tmp, $url, true]; } $path=(string)($args['file_path'] ?? ''); if($path) { if(!is_file($path)) throw new RuntimeException('file_path must be a real local path on the WordPress server. ChatGPT uploaded-file sandbox paths are not accessible here; upload the ZIP to a public HTTPS URL and pass kit_url instead.'); return [$path, $path, false]; } throw new RuntimeException('kit_url is required. Provide a public HTTPS URL to the Elementor Template Kit ZIP. ChatGPT uploaded files/local sandbox paths are not accessible to WordPress.'); }
